# Staqtapp / Monolith VFS Legacy with multi-processing
...
_API_LOCK_PATH = os.path.join(SQTPP_MDL_DIR, 'staqtapp1_2', '.staqtapp.api.lock')
_API_THREAD_LOCK = threading.RLock()
_API_LOCK_STATE = threading.local()

def _is_safe_vfs_name(name):
    return (
        isinstance(name, str)
        and bool(name)
        and name not in {'.', '..'}
...
def _safe_pickle_loads(payload):
    return _RestrictedUnpickler(io.BytesIO(payload)).load()


def _atomic_write_text(path, source):
    if not isinstance(source, str):
        raise TypeError('file source must be a string')
    directory = os.path.dirname(os.path.abspath(path))
    os.makedirs(directory, exist_ok=True)
    descriptor, temporary_path = tempfile.mkstemp(prefix='.staqtapp-', suffix='.tmp', dir=directory)
    try:
        with os.fdopen(descriptor, mode='w', encoding='utf-8') as file_object:
...
def _api_guard():
    with _API_THREAD_LOCK:
        depth = getattr(_API_LOCK_STATE, 'depth', 0)
        if depth:
            _API_LOCK_STATE.depth = depth + 1
            try:
                yield
            finally:
                _API_LOCK_STATE.depth -= 1
            return
        _API_LOCK_STATE.depth = 1
        try:
            with _InterProcessFileLock(_API_LOCK_PATH):
                yield
        finally:
            _API_LOCK_STATE.depth = 0
...
class AbsSmvt(dict):
    ...
    def __init__(self, dt=None):
        ...
        for key, value in dt.items():
            self[key] = type(self)(value) if isinstance(value, dict) else value
...
class Sqtpp():
    ...
    def mcf_sqtpp_rd1(self, src):
        sfCls = SqtppFncs()
        return sfCls.sqtpp_ntr_mvi8_intrpt(src)

    def mcf_vitalvar(self):
        raise NotImplementedError('vitalvar is not implemented in this public release')
    ...
    def mcf_pojishon(self, mode: str, varDat, varNm, dirLst: list):
        ...
        if self._sRtrn in ('FNC-ERR', 'FOO-BAR'):
            self.mcf_err_handler(-1, 'pojishon')
    ...
    def mcf_appvar(self, varNms: list, varDts: list, varLks):
        ...
        if isinstance(varNms, list) and isinstance(varDts, list):
            if not varNms:
                raise ValueError('appvar requires at least one variable name')
            if len(varDts) > len(varNms):
                raise ValueError('appvar received more data values than variable names')
            if len(set(varNms)) != len(varNms):
                raise ValueError('appvar variable names must be unique')
            self._rIntA = 0
            self._rBoolA = True
            ...
            if self._rBoolA:
                ...
                elif self._sRtrn == -8: raise ValueError('appvar cannot overwrite an existing variable')
    ...
    def mcf_genicvar(self, mode: str, vrNm, vrDt, vrId):
        sfCls = SqtppFncs()
        if not sfCls.sqtpp_chars_check(2, vrNm):
            self.mcf_err_handler(8, 'genicvar')
        self._sRtrn = sfCls.sqtpp_lst_gen_lcks(mode, vrNm, vrDt, vrId)
        if self._sRtrn in ('FNC-ERR', 'FOO-BAR'):
            self.mcf_err_handler(-1, 'genicvar')
        if self._sRtrn is None:
            raise NotImplementedError('genicvar is incomplete in this public release')
        return self._sRtrn
    ...
    def mcf_revar(self, isNewSetPth: bool, newVfsFlNm: str, newVfsDirNm: str, newVfsFldrNm: str):
        ...
        sfCls = SqtppFncs()
        if _is_safe_vfs_name(newVfsFlNm):
    ...
    def mcf_err_handler(self, altErrCd: int, clnFnc: str):
        ...
        if altErrCd > 0:
            ...
            elif altErrCd == 4: raise Exception(f'staqtapp1.2 ({clnFnc}) error: invalid or unsafe staqtapp vfs filename')
...
class SqtppFncs(Sqtpp):
    ...
    def sqtpp_vfs_make(self, vfsNm: str, dirNm: str, fldrNm: str) -> int:
        if not _is_safe_vfs_name(vfsNm):
            raise ValueError('invalid or unsafe vfs file name')

        try:
            os.makedirs(f'{SQTPP_MDL_DIR}/staqtapp1_2', exist_ok=True)
    ...
    def sqtpp_change_vfs_path(self, flNm: str, dirNm: str, fldrNm: str):
        ...
        try:
            ...
            for self._sf_rIntA in range(self._sf_rIntB):
                if os.path.basename(self._sf_rLstA[self._sf_rIntA]) == f'{flNm}.sqtpp': self._sf_rBoolA = True
            if self._sf_rBoolA:
                with open(f'{SQTPP_MDL_DIR}/staqtapp1_2/{flNm}.sqtpp', mode='r', encoding='utf-8') as sqtppFl: self._sf_rStrA = sqtppFl.read()
                if self._sf_rStrA.find(f'\n|:{dirNm}<{fldrNm}>\n') > -1:
                    if self._sf_rStrA.find(f'\n_|:{fldrNm}<sub-{fldrNm}>\n') > -1:
                        if self._sf_rStrA.find(f'\n__|:sub-{fldrNm}<') > -1:
                            if self._sf_rStrA.find(f'\n___|:tqpt-{fldrNm}<') > -1:
                                self._sf_rStrA = None
                                self.sqtpp_file(True, f'{SQTPP_MDL_DIR}/staqtapp1_2/sqtpp1_2.stg', f'{flNm}:{dirNm}:{fldrNm}:sub-{fldrNm}:tqpt-{fldrNm}')
                            else:
                                return -5
                        else:
                            return -4
    ...
    def sqtpp_vfs_display_file_struct(self):
        self.sqtpp_tqpt_path(False, None)
        if not self._sf_sSrc:
            return {}
        parts = self._sf_sSrc.split(':')
        if len(parts) != 5 or not _is_safe_vfs_name(parts[0]):
            raise ValueError('the vfs path settings file is malformed')
        vfs_path = os.path.join(SQTPP_MDL_DIR, 'staqtapp1_2', f'{parts[0]}.sqtpp')
        return {
            'vfs_file': vfs_path,
            'directory': parts[1],
            'folder': parts[2],
            'subfolder': parts[3],
            'tqpt': parts[4],
            'tpqt': parts[4].replace('tqpt', 'tpqt', 1),
            'exists': os.path.isfile(vfs_path),
        }
                ...
                if self.sqtpp_vfs_tqpt_file(True) != -1:
                    if mode == 1:
                        ...
                        if len(self._sf_sVd) > 0:
                            ...
                            if mode == 2: self.sqtpp_delta_prle_decd(_safe_pickle_loads(base64.b64decode(self._sf_sVd.encode('utf-8'))))
                            else: self._sf_rLstA = _safe_pickle_loads(base64.b64decode(self._sf_sVd.encode('utf-8')))
                            self._sf_sSrc = None
                            self._sf_sQp = None
        ...
        try:
            if self.sqtpp_set_vfs_file() == 1:
                if self.sqtpp_vfs_tqpt_file(True) != -1:
                    if self.sqtpp_vfs_tpqt_file(True) != -1:
                        self._sf_rBoolA = False
                        if self._sf_sSrc.find(f'<sbf-{self._sf_sVfsFldr}-svvs:') > -1 and self.sqtpp_svvs_list(varNm) == 8:
                            ...
                            if len(self._sf_rLstA) > 1:
                                ...
                                if newVarNm > 0:
                                    ...
                                    if len(self._sf_rLstC) > 0:
                                        self._sf_sSrc = self._sf_sSrc.replace(self._sf_rStrB, self._sf_sQp)
                                        self.sqtpp_silent_lock_remove(False, self._sf_rLstC)
                                    self._sf_rStrA = self.sqtpp_vfs_sub_file(False, False, 'svvs', None)
                    ...
                    if not self._sf_rBoolA:
                        ...
                        if self._sf_rIntD > 1:
                            ...
                            if self._sf_rIntE > 0:
                                ...
                                self._sf_sQp = None
                                self.sqtpp_reset_slots(False)
                                with open(f'{SQTPP_MDL_DIR}/staqtapp1_2/sqtpp1_2.stg', 'r', encoding='utf-8') as stgFl: self._sf_rLstA = stgFl.read().split(':')
                    ...
                    if self.sqtpp_var_value(trNm):
                        ...
                        if self.sqtpp_smvt_chars_check(str(newSmvBrnchId)):
                            if self.sqtpp_smvt_chars_check(str(newSmvBrnchVl)):
                                ...
                                self._sf_sRtrn = self.sqtpp_mv_prometes_kn(False,'ab',str(_safe_pickle_loads(str.encode(twRwSrc.replace('☆','\n')))),str(smvBrncId),str(newSmvBrnchId),str(newSmvBrnchVl))
        ...
        try:
            ...
            if self.sqtpp_set_vfs_file() == 1:
                if self.sqtpp_vfs_tqpt_file(True) != -1:
                    if self.sqtpp_var_value(trNm):
                        twRwSrc = self._sf_sVd[4:len(self._sf_sVd)-2]
                        twRwSrc = str(_safe_pickle_loads(str.encode(twRwSrc.replace('☆','\n')))).replace(' ','')
                        self._sf_sVd = None
    ...
    def sqtpp_delta_prle_encd(self, blLst: list):
        ...
        for self._sf_rIntA in range(1, self._sf_rIntB):
            if self._sf_rLstB[self._sf_rIntA] == self._sf_rLstB[self._sf_rIntA-1]: self._sf_rIntC+=1
                ...
                self._sf_rIntC = 1
        self._sf_rLstA.append((self._sf_rLstB[-1], self._sf_rIntC))
                    ...
                    while self._sf_rIntA < self._sf_rIntB:
                        ...
                        if self.sqtpp_var_value(varNmLst[self._sf_rIntA]):
                            if self._sf_sVd.find('(c__main__') > -1 and self._sf_sVd.find('c__builtin__') > -1:
                                try:
                                    self._sf_sVd = str(_safe_pickle_loads(str.encode(self._sf_sVd)))
                                except Exception as e:
                                    self._sf_rBoolA = True
                            if not self._sf_rBoolA:
                                if not isRgx:
    ...
    def sqtpp_tqpt_path(self, isSetPth: bool, pth: str):
        settings_path = f'{SQTPP_MDL_DIR}/staqtapp1_2/sqtpp1_2.stg'
        if isSetPth:
            self.sqtpp_file(True, settings_path, pth)
        elif os.path.isfile(settings_path):
            self.sqtpp_file(False, settings_path, None)
        else:
            self._sf_sSrc = ''

    def sqtpp_file(self, isWrt: bool, pth: str, src: str):
        if isWrt:
            _atomic_write_text(pth, src)
        else:
            with open(pth, mode='r', encoding='utf-8') as file_object:
                self._sf_sSrc = file_object.read()
    ...
    def sqtpp_silent_lock_add(self, varNm: str, lckNmLst: list):
        ...
        try:
            ...
            raise RuntimeError('failed to add silent lock metadata') from error
    ...
    def sqtpp_silent_lock_remove(self, isStlkVar: bool, varNmLst: list):
        ...
        try:
            ...
            raise RuntimeError('failed to remove silent lock metadata') from error
    ...
    def sqtpp_lambda_lambda_lambda_clsr(self, lmbNm: str, lmbPrms: list):
        ...
        if os.path.isfile(f'{SQTPP_MDL_DIR}/sqtpp1_2_LMB.py'):
            ...
            if len(self._sf_rLstB) > 0: self._sf_rBoolC = False
                ...
                _atomic_write_text(f'{SQTPP_MDL_DIR}/sqtpp1_2_LMB.py', self._sf_rStrA + '\n    ' + '#CTMN-LLL<' + self._sf_sStrX + ':' + lmbNm + '>\n    ' + lmbNm + ' = ' + self._sf_rStrD + '\n\ndef sqtpp_lll_run(lName: str, lPrms: list):\n    lllCls = SqtppLLL_Joints()\n    return lllCls.sqtpp_lambda_lambda_lambda_set_slots_attrs_and_run(lName, lPrms)')
                self._sf_rBoolC = True
        else:
            ...
            self._sf_rLstB.append('\n\ndef sqtpp_lll_run(lName: str, lPrms: list):\n    lllCls = SqtppLLL_Joints()\n    return lllCls.sqtpp_lambda_lambda_lambda_set_slots_attrs_and_run(lName, lPrms)')
            _atomic_write_text(f'{SQTPP_MDL_DIR}/sqtpp1_2_LMB.py', ''.join(self._sf_rLstB))
            self._sf_rBoolC = True
            self._sf_rLstB = None
        if not self._sf_rBoolC:
            import sqtpp1_2_LMB
                            ...
                            if vrDt.find('{') > -1 or vrDt.find('}') > -1:
                                if vrDt.find('[') > -1 or vrDt.find(']') > -1:
                                    if not os.path.isfile(f'{SQTPP_MDL_DIR}/__sqtpp__default__omega__key__.txt'):
                                        ...
                                        _atomic_write_text(f'{SQTPP_MDL_DIR}/__sqtpp__default__omega__key__.txt', '\n'.join(self._sf_rLstC))
    ...
    def sqtpp_ntr_tree_build_mdl(self):
        ...
        _atomic_write_text(f'{SQTPP_MDL_DIR}/sqtpp1_2_NTR.py', self._rg_rLstA)
        self._rg_rLstA = None
    ...
    def sqtpp_registry_homer_build(self, addRegKy: bool, regKyNm: str, regKyDt, schm: str):
        ...
        _atomic_write_text(f'{SQTPP_MDL_DIR}/sqtpp1_2_REG.py', self._rg_rLstA)
        self._rg_rStrA = None
        self._rg_rLstA = None
    ...
    def sqtpp_registry_homer_add_key(self, regKyNm: str, regKyDt, schNm: str):
        ...
        try:
            if self._rg_rStrA.find(f':008S9:{schNm}={schNm}=(') > -1:
                ...
                self._rg_rStrB = None
                self._rg_rStrC = None
                _atomic_write_text(f'{SQTPP_MDL_DIR}/sqtpp1_2_REG.py', self._rg_rStrA)
...
def _call_api_method(method_name, *args, **kwargs):
    with _api_guard():
        return getattr(Sqtpp(), method_name)(*args, **kwargs)

def sqtpp_rd1(src):
    """Run the internal diagnostic reader."""
    return _call_api_method('mcf_sqtpp_rd1', src)

def makevfs(vfsFileName: str, directoryName: str, folderName: str):
    """Create a VFS file and select its TQPT path."""
    return _call_api_method('mcf_makevfs', vfsFileName, directoryName, folderName)

def setpath(vfsFileName: str, directoryName: str, folderName: str):
    """Select an existing VFS TQPT path."""
    return _call_api_method('mcf_setpath', vfsFileName, directoryName, folderName)

def corevar(mode: int, varName: str, booleanList: list):
    """Write or read a packed boolean variable."""
    return _call_api_method('mcf_corevar', mode, varName, booleanList)

def addvar(varName: str, varData: str):
    """Add one variable to the selected VFS."""
    return _call_api_method('mcf_addvar', varName, varData)

def appvar(varNames: list, varDatas: list, varLocks):
    """Add multiple variables and optional lock records."""
    return _call_api_method('mcf_appvar', varNames, varDatas, varLocks)

def renamevar_stx(varName: str, newVarName):
    """Rename a normal or stalked variable."""
    return _call_api_method('mcf_renamevar_stx', varName, newVarName)

def removevar(varName: str):
    """Remove a variable from the selected VFS."""
    return _call_api_method('mcf_removevar', varName)

def listvars() -> list:
    """Return variable names in the selected VFS."""
    return _call_api_method('mcf_listvars')

def listfiles():
    """Return the active VFS path structure."""
    return _call_api_method('mcf_list_files')

def lambdalist(asComplete: bool):
    """Return stored lambda entries."""
    return _call_api_method('mcf_lambdalist', asComplete)

def genicvar(mode: str, varName, varData, varId: list):
    """Access the experimental generated-comprehension facility."""
    return _call_api_method('mcf_genicvar', mode, varName, varData, varId)

def joinvars(newVarName: str, varNames: list):
    """Join variable payloads into a target variable."""
    return _call_api_method('mcf_joinvars', newVarName, varNames)

def changevar(varName: str, newVarData: str):
    """Replace variable data."""
    return _call_api_method('mcf_changevar', varName, newVarData)

def addtree_stx(treeName: str, initialTreePathList: list):
    """Create a stored SMVT tree."""
    return _call_api_method('mcf_addtree_stx', treeName, initialTreePathList)

def addbranch_stx(treeName: str, branchKey, newBranchKey, newBranchValue):
    """Add a branch to a stored SMVT tree."""
    return _call_api_method('mcf_addbranch_stx', treeName, branchKey, newBranchKey, newBranchValue)

def getbranch_stx(isAlf: bool, treeName: str, branchKey):
    """Read a branch from a stored SMVT tree."""
    return _call_api_method('mcf_getbranch_stx', isAlf, treeName, branchKey)

def vardata_stx(isRegex: bool, varNameList: list, search: str) -> list:
    """Return variables whose data matches a search."""
    return _call_api_method('mcf_vardata_stx', isRegex, varNameList, search)

def lockvar(varName: str, fncName):
    """Attach one or more lock names to a variable."""
    return _call_api_method('mcf_lockvar', varName, fncName)

def locklist(varName: str) -> list:
    """Return lock names attached to a variable."""
    return _call_api_method('mcf_locklist', varName)

def lockdel(isDelAll: bool, varName: str, fncName):
    """Remove selected or all lock names from a variable."""
    return _call_api_method('mcf_lockdel', isDelAll, varName, fncName)

def keyvar(varName: str, fncName) -> bool:
    """Check whether a lock name authorizes a variable."""
    return _call_api_method('mcf_keyvar', varName, fncName)

def darkvar():
    """Run dark-variable maintenance."""
    return _call_api_method('mcf_darkvar')

def revar(isNewSetVfsPath, newVfsFileName, newVfsDirName, newVfsFldrName):
    """Create a VFS from locked variables."""
    return _call_api_method('mcf_revar', isNewSetVfsPath, newVfsFileName, newVfsDirName, newVfsFldrName)

def findvar(varName: str) -> bool:
    """Check whether a variable exists."""
    return _call_api_method('mcf_findvar', varName)

def findvar_stx(varNameList: list, stalkVarName: str) -> list:
    """Locate multiple variables with optional stalk metadata."""
    return _call_api_method('mcf_findvar_stx', varNameList, stalkVarName)

def loadvar(isAllNumbers: bool, varName: str, mode: str):
    """Load variable data as a deque or string."""
    return _call_api_method('mcf_loadvar', isAllNumbers, varName, mode)

def stalkvar(varName: str, varData: str):
    """Append a stalked variable state."""
    return _call_api_method('mcf_stalkvar', varName, varData)

def lambdavar(lambdaName: str, lambdaParams: list):
    """Execute a stored lambda variable."""
    return _call_api_method('mcf_lambdavar', lambdaName, lambdaParams)

def registry(isRead: bool, keyName: str, keyData, harpSchema: str):
    """Read or write a registry key through a harp schema."""
    return _call_api_method('mcf_lll_registry', isRead, keyName, keyData, harpSchema)

def pojishon(mode: str, varData, varName, dirList: list):
    """Run a Pojishon VFS operation."""
    return _call_api_method('mcf_pojishon', mode, varData, varName, dirList)
...
def invoke_api(api_name, args=(), kwargs=None):
    """Invoke a named public API using a multiprocessing-pickleable request."""
...
def _normalize_api_call(call):
    ...
    if isinstance(call, (tuple, list)):
        if len(call) == 1:
            return call[0], (), {}
        if len(call) == 2:
            return call[0], tuple(call[1]), {}
        if len(call) == 3:
            return call[0], tuple(call[1]), dict(call[2])
    raise TypeError('each API call must be a mapping or a 1-3 item tuple/list')

def _invoke_api_call(call):
    api_name, args, kwargs = _normalize_api_call(call)
    return invoke_api(api_name, args, kwargs)

def map_api_calls(calls, processes=None, start_method='spawn', chunksize=1):
    """Execute API requests in worker processes while serializing VFS access."""
    normalized_calls = [_normalize_api_call(call) for call in calls]
    ...
    with context.Pool(processes=processes) as pool:
        return pool.map(_invoke_api_call, normalized_calls, chunksize)

__all__ = tuple(_PUBLIC_API) + ('invoke_api', 'map_api_calls')
